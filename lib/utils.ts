...
export function exportBoardToJson(boardData: object, filename = 'driftboard-export.json') {
    ...
    if (part && urlRegex.test(part)) {
      result.push(
        React.createElement('a', {
          ...
          className: 'text-blue-600 hover:text-blue-800 hover:underline transition-colors duration-200 font-medium cursor-pointer decoration-1 underline-offset-2'
        }, part)
      );
    } else if (part) {
