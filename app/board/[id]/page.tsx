...
export default function BoardPage() {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-lg text-muted-foreground font-serif">Loading board...</div>
        </div>
      </div>
    )
  }
  ...
  if (!board) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-lg text-muted-foreground font-serif">Board not found</div>
      </div>
    )
  }
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-2 md:gap-4">
            <Button variant="ghost" size="sm" onClick={() => router.push("/")} className="gap-2 shrink-0">
              <ArrowLeft className="h-4 w-4" />
              <span className="hidden sm:inline">Back to Boards</span>
              <span className="sm:hidden">Back</span>
            </Button>
            <div className="flex-1 min-w-0">
              <h1 className="text-lg md:text-2xl font-bold text-foreground font-sans truncate">{board.title}</h1>
              {board.description && (
                <p className="text-xs md:text-sm text-muted-foreground font-serif mt-1 truncate">{board.description}</p>
              )}
            </div>
            <CreateListDialog boardId={boardId} listsCount={lists.length} onListCreated={handleCreateList} />
          </div>
        </div>
      </header>
        ...
        {lists.length === 0 ? (
          <div className="flex items-center justify-center min-h-[60vh]">
            <EmptyState
              ...
              action={<CreateListDialog boardId={boardId} listsCount={0} onListCreated={handleCreateList} />}
            />
          </div>
        ) : (
