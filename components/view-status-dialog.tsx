...
export function ViewStatusDialog({
  ...
  const handleRestore = async (card: Card) => {
    ...
    const parentListIsActive = activeLists.some(
  ...
  const handleReparentConfirm = async (newListId: string) => {
    ...
    try {
      ...
      await cardService.updateCard(cardToReparent.id, user.uid, {
        listId: newListId,
        status: "active",
      });
            ...
            {loading ? (
              ...
              <ul className="space-y-2">
                {cards.map((card) => (
                  <li
                    key={card.id}
                    className="p-3 border rounded-md bg-card flex justify-between items-center gap-4"
                  >
