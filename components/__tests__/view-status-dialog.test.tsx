  ...
  ReparentCardDialog: ({
    ...
    isOpen ? (
      <div data-testid="reparent-dialog">
        <button type="button" onClick={() => onConfirm("new-list-id")}>
          Confirm Reparent
        </button>
...
describe("ViewStatusDialog", () => {
  ...
  it("renders trigger button", () => {
    ...
    expect(
      screen.getByRole("button", { name: /view done cards/i }),
    ).toBeInTheDocument();
  });
  ...
  it("fetches and displays cards when dialog opens", async () => {
    ...
    await waitFor(() => {
      expect(cardService.getCardsByStatus).toHaveBeenCalledWith(
        "board-1",
        "done",
      );
  ...
  it("shows loading skeletons while fetching data", async () => {
    ...
    const listsPromise = new Promise<List[]>((resolve) => {
  ...
  it("renders view and restore buttons for each card", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByRole("button", { name: /restore/i })).toHaveLength(
        2,
      );
    });
  });
  ...
  it("restores card directly when parent list is active", async () => {
    ...
    await waitFor(() => {
      expect(cardService.updateCardStatus).toHaveBeenCalledWith(
        "card-1",
        "test-user-id",
        "active",
      );
  ...
  it("handles fetch error gracefully", async () => {
    ...
    (cardService.getCardsByStatus as any).mockRejectedValue(
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith(
        "Error fetching data for dialog:",
        expect.any(Error),
      );
    });
