...
describe("CommentItem URL Linking", () => {
  ...
  it("renders multiple URLs as separate clickable links", () => {
    ...
    render(
      <CommentItem
        comment={commentWithMultipleUrls}
        onCommentUpdated={mockOnCommentUpdated}
      />,
    );
  ...
  it("handles comment content with no URLs normally", () => {
    const commentNoUrl = createMockComment(
      "This is just plain text without any links",
    );

    render(
      <CommentItem
        comment={commentNoUrl}
        onCommentUpdated={mockOnCommentUpdated}
      />,
    );
  ...
  it("preserves whitespace and line breaks while rendering URLs", () => {
    const commentWithWhitespace = createMockComment(
      "Line 1\nhttps://example.com\n\nLine 3",
    );
