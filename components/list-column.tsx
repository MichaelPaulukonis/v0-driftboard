...
export function ListColumn({
  ...
  const { user } = useAuth();
  const { instanceId, can } = useBoardContext();
  ...
  const canReorderLists = can("reorderLists");
  const canUpdateList = can("updateList");
  const canDeleteList = can("deleteList");
  const canCreateCard = can("createCard");
  ...
  }, [list.id, instanceId, canReorderLists, canUpdateList, can]);
  ...
  return (
    <>
      <div
        className={`flex-shrink-0 w-72 md:w-80 relative ${isBeingDragged ? "opacity-40" : ""}`}
      >
        <UICard
          ref={listRef}
        ...
        >
            ...
            <div className="flex items-center justify-between">
              {isEditing ? (
                ...
                <>
                  <div
                    className="flex-1"
                    onDoubleClick={() => canUpdateList && setIsEditing(true)}
                  >
          ...
          <ColumnContext.Provider value={columnContextValue}>
            ...
            >
              ...
              {list.items.length === 0 && (
                <div className="text-center py-6 text-muted-foreground font-serif text-sm">
                  No cards yet
                </div>
              )}
              {canCreateCard && (
                <CreateCardDialog
                  listId={list.id}
                  cardsCount={list.items.length}
                  onCardCreated={onListUpdated}
                />
              )}
            </CardContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="font-serif">
              Are you sure you want to delete "{list.title}"? This will move the
              list and its {list.items.length} active card(s) to the deleted
              view.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
