...
type State = { type: "idle" } | { type: "dragging" };
...
export function CardItem({
  ...
  const canReorderCards = can("reorderCards");
  const canUpdateCard = can("updateCard");
  const canDeleteCard = can("deleteCard");
  ...
  }, [card.id, instanceId, listId, canReorderCards]);

  const handleSetStatus = async (status: Card["status"]) => {
    if (!user || !canUpdateCard) return;
    setLoading(true);
    try {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="font-serif">
              Are you sure you want to delete "{card.title}"? This will move the
              card to the deleted items view, where it can be restored later.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
